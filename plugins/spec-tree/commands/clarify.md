...
Then proceed to gap_analysis with their response.
...
<execution>
When the user selects "Start working":

1. Summarize the gathered context back to the user in 2-3 sentences so they can confirm understanding
2. Identify the concrete deliverable (code, document, analysis, etc.)
3. Proceed with the task using all available tools — read files, run tests, write code, etc.
4. If new ambiguity surfaces during execution, ask before assuming
...
7. Summarize gathered context, confirm understanding, then execute the task per the execution section
